...
class WheelChairVisualization:
    ...
    def _draw_robot(self, position, direction):
        "Returns a polygon representing a robot with the specified parameters."
        x, y = position.getX(), position.getY()
        d1 = direction + math.radians(165)
        d2 = direction - math.radians(165)
        x1, y1 = self._map_coords(x, y)
        x2, y2 = self._map_coords(x + 0.6 * math.cos(d1),
                                  y + 0.6 * math.sin(d1))
        x3, y3 = self._map_coords(x + 0.6 * math.cos(d2),
                                  y + 0.6 * math.sin(d2))
        return self.w.create_polygon([x1, y1, x2, y2, x3, y3], fill="red")
